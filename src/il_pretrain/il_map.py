...
def save_model(net_model, global_iter, global_step, model_dir, is_best):
    ...
    # torch.save(data_to_save, os.path.join(self.model_dir, 'model_best.pth'))
...
def parse_arguments():
    ...
    parser.add_argument('--save_dir', type=str, default='./il_pretrain_map')
...
def main():
    ...
    for iter in tqdm(range(global_iter, args.max_iters), desc='iter'):
        ...
        for i_batch, (rgbs, l_maps, s_maps, actions) in enumerate(tqdm(train_dataloader,
            ...
            ep_l_maps = l_maps.float().to(args.device).permute(1, 0, 2, 3, 4)
            ep_l_maps = imagenet_rgb_preprocess(ep_l_maps[:-1, :, ...], device=args.device)

            ep_s_maps = s_maps.float().to(args.device).permute(1, 0, 2, 3, 4)
            ep_s_maps = imagenet_rgb_preprocess(ep_s_maps[:-1, :, ...], device=args.device)

            tgt_actions = actions.permute(1, 0).to(args.device)
            ...
            for start in range(0, args.traj_len, args.rnn_seq_len):
                ...
                it_l_maps = ep_l_maps[start: end]
                it_s_maps = ep_s_maps[start: end]
                it_actions = tgt_actions[start:end]
                pred_actions, _, hidden_state = net_model(it_l_maps, it_s_maps,
